...
class ClubOperation(object):
    ...
    def get_club_player_list(redis, club_number):
        ...
        for item in accounts:
            player = {}
            nickname, headImgUrl, account, last_logout_date = redis.hmget(redis.get(FORMAT_ACCOUNT2USER_TABLE % item), 'nickname', 'headImgUrl', 'account', "last_logout_date")
...
def create_club(redis, session):
    ...
    try:
        ...
        return {"code": 0, "msg": "创建成功", "club_number": next_id}
    except Exception as err:
        traceback.print_exc()
...
def apply_club(redis, session):
    ...
    # 检查俱乐部是否存在
    if redis.exists(CLUB_ATTR % club_number):
        nickname, headImgUrl, account = redis.hmget(redis.get(FORMAT_ACCOUNT2USER_TABLE % account), 'nickname', 'headImgUrl', 'account')
        # 增加用户到这个俱乐部申请表中
        redis.sadd(CLUB_AUDI_LIST%club_number, "%s|---|%s|---|%s|---|%s" % (account,nickname, headImgUrl, 0))
        return {"code": 0, 'msg': '成功'}
...
def doclub_getRoomList(redis, session):
    ...
    for roomNum in roomLists:
        ...
        gameName, dealer, playerCount, maxPlayer, gameid, baseScore, ruleText, gameState = redis.hmget(ROOM2SERVER%(roomNum), ('gameName', 'dealer', 'playerCount', 'maxPlayer', 'gameid','baseScore', 'ruleText', "gameState"))
        if not ruleText:
            continue
        if not baseScore:
            baseScore = 1
        ...
        result = {'gameName':gameName, 'dealer':dealer, 'playerCount':playerCount, 'maxPlayer':maxPlayer, 'roomNum':roomNum, 'gameid':gameid, 'ruleText':ruleText.replace("\n", '|'), 'baseScore':int(baseScore), 'gameType': int(gameType), "gameState": gameState, "auto_id": auto_id}
        result["players_list"] = []
    ...
    return {'code':0, 'roomData':roomDatas, "data": {"playerNumber": playersNumber, "active": onlineNumber, "roomNumber": roomlistNumber, "roomActiveNumber": roomActiveNumber, "club_number": club_number}}
...
def club_into(redis, session):
    ...
    if redis.exists(CLUB_ATTR % club_number):
        ...
        roomActiveNumber = 0
        for roomNum in roomLists:
            if not redis.exists(ROOM2SERVER % (roomNum)):
                continue
            print '[getRoomList][info] ROOM2SERVER[%s]' % ROOM2SERVER % (roomNum)
            gameName, dealer, playerCount, maxPlayer, gameid, baseScore, ruleText = redis.hmget(ROOM2SERVER % (roomNum),
                                                                                                ('gameName', 'dealer',
                                                                                                 'playerCount',
                                                                                                 'maxPlayer', 'gameid',
                                                                                                 'baseScore',
            ...
            if otherRoomsDict.get("roomNum"):
                roomTable = otherRoomsDict.get("roomNum")
                try:
                    gameType = int(redis.hget(roomTable, "gameType"))
                except Exception as err:
                    redis.lrem(otherRooms, roomTable)
            if int(gameType) == 1:
                roomActiveNumber += 1
        ...
        return {"code":0, "msg": '成功', "data": {"playerNumber": playersNumber, "active": onlineNumber, "roomNumber": roomlistNumber, "roomActiveNumber": roomActiveNumber, "club_number": club_number}, "prompt": msg, "isManager": isManager }
