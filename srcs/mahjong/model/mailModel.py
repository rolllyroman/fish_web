...
def create_active(redis,agentId,messageInfo,create_type='HALL'):
    """
    创建一条新活动
    """
    ...
    active_id = redis.incr(FORMAT_ACTIVE_COUNT_TABLE)
    #读取的url
    read_url = '/admin/notice/active/read?id=%s&agentId=%s&action=%s'%(active_id,agentId,create_type)
    active_info = {
                'read': '0',
                'id':   active_id,
                'link': read_url,
                'sort': convert_util.to_int(messageInfo['sort']),
                'title': messageInfo['title'],
                'imgPath1': messageInfo['imgPath1'],
                'imgPath2': messageInfo['imgPath2'],
                'imgPath3': messageInfo['imgPath3'],
                'time': timeStr,
                'status': '0'
    }
    pipe = redis.pipeline()
    try:
        ...
        pipe.hmset(active_table, active_info)
        #放进公告列表
        pipe.lpush(active_list_table, active_id)
        #放进代理列表
        agent_send_active_table = FORMAT_ACTIVE_SEND_MESSAGE_LIST%(agentId)
        pipe.lpush(agent_send_active_table,active_id)
        #放进代理玩家列表
        #push2userMsgTable(redis,agentId,active_id)
    except Exception,e:
        log_util.debug('[FUNC][create_active][error] reason[%s]'%(e))
        return None

    log_util.debug('[FUNC][create_active][info] messageInfo[%s]'%(messageInfo))
    return pipe.execute()
...
def get_active_list(redis,session,lang,agentId,hall_type='HALL'):
    """
    获取活动列表
    """
    log_util.debug('systemId[%s] agentId[%s] HALL[%s]'%(systemId,agentId,hall_type))
    if int(agentId) == systemId:
        if hall_type == 'HALL':
            active_ids = redis.lrange(FORMAT_GAMEHALL_ACTIVE_LIST_TABLE,0,-1)
        else:
            active_ids = redis.lrange(FORMAT_FISHHALL_ACTIVE_LIST_TABLE,0,-1)
    else:
        active_ids = redis.lrange(FORMAT_ACTIVE_SEND_MESSAGE_LIST%(agentId),0,-1)

    log_util.debug('[try get_active_list] agent_id[%s] active_id[%s] hall_type[%s] '%(agentId,active_ids,hall_type))
    ...
    for active_id in active_ids:
        ...
        active_info['op'] = []
        # sessiob['access']
        for access in access_module.ACCESS_GAME_ACTIVE_LIST:
            if access.url in eval(session['access']):
                if access.url[-4:] == 'push':
                    active_info['op'].append({'url':access.url+"/{}".format(hall_type),'txt':'推送' \
                                if active_info['status'] == '0' else '取消推送','method':access.method})
                else:
                    active_info['op'].append({'url':access.url+"/{}".format(hall_type),'method':access.method,'txt':access.getTxt(lang)})
        active_list.append(active_info)

    active_list = sorted(active_list, key=itemgetter('time'),reverse=True)
    return {'data':active_list,'count':len(active_list)}
