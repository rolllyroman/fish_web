...
@admin_app.post('/goods/currency/exchange/modify')
@checkAccess
def createItem(redis,session):
    ...
    redis.hmset("currency:change:course:%s:hesh"%cid,{
        "name":name,
        "cost":cost,
        "cost_type":cost_type,
        "gain":gain,
    ...
    redirect(BACK_PRE+'/goods/currency/exchange/list?post_res=1')
    

@admin_app.get('/goods/currency/exchange/modify')
@checkAccess
def exchangeModify(redis,session):
    ...
    info = {
        'title'                  :           '修改兑换套餐',
        'submitUrl'              :           BACK_PRE + "/goods/currency/exchange/modify",
        'STATIC_LAYUI_PATH'      :           STATIC_LAYUI_PATH,
        'STATIC_ADMIN_PATH'      :           STATIC_ADMIN_PATH
    }

    return template('admin_goods_currency_exchange_modify',info=info,lang=lang,RES_VERSION=RES_VERSION,cid=cid,**dic)

@admin_app.get('/goods/currency/exchange/del')
@checkAccess
def createItem(redis,session):
    '''
        删除兑换套餐
    ...
    redis.delete("currency:change:course:%s:hesh"%cid)
    redis.srem("currency:change:course:set",cid)

    redirect(BACK_PRE+'/goods/currency/exchange/list?post_res=6')

@admin_app.get('/goods/currency/exchange/create')
@checkAccess
def createItem(redis,session):
    '''
        创建兑换套餐
    ...
    info = {
            ...
            'STATIC_LAYUI_PATH'      :       STATIC_LAYUI_PATH,
            'STATIC_ADMIN_PATH'      :       STATIC_ADMIN_PATH,
            'submitUrl'              :       BACK_PRE + "/goods/currency/exchange/create"
    }

    return template('admin_goods_currency_exchange_create',info=info,lang=lang,RES_VERSION=RES_VERSION)

@admin_app.post('/goods/currency/exchange/create')
@checkAccess
def createItem(redis,session):
    '''
        创建兑换套餐
    ...
    if not all([cid,name,cost_type,cost,gain_type,gain]):
        redirect(BACK_PRE+'/goods/currency/exchange/list?post_res=3')

    if cid in redis.smembers("currency:change:course:set"):
        redirect(BACK_PRE+'/goods/currency/exchange/list?post_res=3')
    ...
    redis.sadd("currency:change:course:set",cid)
    redirect(BACK_PRE+'/goods/currency/exchange/list?post_res=4')

@admin_app.get('/goods/currency/exchange/list')
@checkAccess
def do_inner(redis,session):
    ...
    if isList:
        cids = redis.smembers("currency:change:course:set")
        data = []
        for cid in cids:
            dic = redis.hgetall("currency:change:course:%s:hesh"%cid)
            dic['cid'] = cid
        ...
        info = { 
            'title'                  :           '兑换列表',
            'listUrl'                :           BACK_PRE+'/goods/currency/exchange/list?list=1',
            'STATIC_LAYUI_PATH'      :           STATIC_LAYUI_PATH,
            'STATIC_ADMIN_PATH'      :           STATIC_ADMIN_PATH
        }   

        return template('admin_goods_currency_exchange_list',info=info,lang=lang,RES_VERSION=RES_VERSION,post_res=post_res)
