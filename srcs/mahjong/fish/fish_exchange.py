...
@fish_app.post('/currency/exchange')
@allow_cross
def do_inner(redis,session):
    ...
    box_pwd = redis.hget('users:%s'%uid,'box_pwd')
    if not box_pwd:
        return {'code':1,'msg':'请先设安全箱密码！'}

    formatInt = lambda (x,y):[int(x or 0),int(y or 0)]
    cost,gain = formatInt(redis.hmget('currency:change:course:%s:hesh'%cid,'cost','gain'))
    cost_type,gain_type = formatInt(redis.hmget('currency:change:course:%s:hesh'%cid,'cost_type','gain_type'))
    cur_coin,cur_diamond = formatInt(redis.hmget('users:%s'%uid,'coin','diamond'))
    # 消耗钻石
    if cost_type == 1:
        cur_diamond = int(redis.hget('box:user:%s:hesh'%uid,cost_type) or 0)
        if cur_diamond < cost:
            return {'code':'-1','msg':'钻石不够!'}

        # 钻石换金币
        if gain_type == 2:
            redis.hincrby('box:user:%s:hesh'%uid,gain_type,gain)
            redis.incrby('fish:diamond2coin:sum',gain) 
        else:
            redis.hincrby('box:user:%s:hesh'%uid,gain_type,gain)
        #消耗
        redis.hincrby('box:user:%s:hesh'%uid,cost_type,-cost)

    # 消耗金币 
    elif cost_type == 2:
        if cur_coin < cost:
            return {'code':'-1','msg':'金币不够!'}

        # 金币换钻石
        if gain_type == 1:
            redis.hincrby('users:%s'%uid,'diamond',gain)
            redis.hincrby('box:user:%s:hesh'%uid,gain_type,gain)
        else:
            redis.hincrby('box:user:%s:hesh'%uid,gain_type,gain)
        # 消耗
        redis.hincrby('users:%s'%uid,'coin',-cost)

    # 消耗其他道具
    else:
        cur_num = redis.hget('player:uid:%s:hesh'%uid,cost_type)
        if int(cur_num or 0)<cost:
            return {'code':'-1','msg':'数量不够!'}
        # 换钻石
        if gain_type == 1:
            redis.hincrby('box:user:%s:hesh'%uid,gain_type,gain)
            #redis.hincrby('users:%s'%uid,'diamond',gain)
        # 换金币
            ...
            redis.hincrby('box:user:%s:hesh'%uid,gain_type,gain)
            #redis.hincrby('users:%s'%uid,'coin',gain)
        # 换其他
        else:
            redis.hincrby('box:user:%s:hesh'%uid,gain_type,gain)
        # 消耗
        cur_num = redis.hincrby('player:uid:%s:hesh'%uid,cost_type,cost)

    coin,diamond = redis.hmget('users:%s'%uid,'coin','diamond')
    return {'code':'0','msg':'兑换成功!','coin':coin,'diamond':diamond}
