...
def vip_charge_record(redis,uid,price):
    charge_sum = int(redis.hincrby('users:%s'%uid,'vip_charge',price) or 0)

    if charge_sum >= 1000: 
        redis.hset('users:%s'%uid,'vip_level',1)
    if charge_sum >= 3000: 
        redis.hset('users:%s'%uid,'vip_level',2)
    if charge_sum >= 6000: 
        redis.hset('users:%s'%uid,'vip_level',3)
    if charge_sum >= 10000: 
        redis.hset('users:%s'%uid,'vip_level',4)
    if charge_sum >= 20000: 
        redis.hset('users:%s'%uid,'vip_level',5)
...
def do_pay_record(redis,user_id,user_table,coin,price):
    ...
    try:
        ...
        try:
            pipe.hincrbyfloat(user_table,'recharge_coin_total',int(price)*0.01)
        except:
            pipe.hincrbyfloat(user_table,'recharge_coin_total',float(price)*0.01)
        pipe.incrbyfloat(FISH_SYSTEM_RECHARGE_TOTAL,price)
        pipe.hincrbyfloat(FISH_SYSTEM_DATE_RECHARGE_TOTAL%(today),'recharge_coin_total',int(price)*0.01)
...
def do_add_fishcoin_2_members(redis,transNo):
    ...
    groupId,ori_coin = redis.hmget(userTable, 'parentAg','coin')
    log_util.debug('[%s][wechatPay] recharge roomcards[%s][%s] to account[%s] success'%(curTime,cardNums,ori_coin,memberAccount))
    return
