...
def do_buyCoin(redis,session):
    ...
    :params cards 购买金币数
    """
    fields = ('sid','cards','price','goodId','uid')
    for field in fields:
        exec('%s = request.forms.get("%s","").strip()'%(field,field))

    SessionTable,account,uid,verfiySid = getInfoBySid(redis,sid)

    box_pwd,box_lock = redis.hmget('users:%s'%uid,'box_pwd','box_lock')
    # 如果保管箱未设密码则不允许购买
    if not box_pwd: 
        return {'code':1,'msg':'请先设安全箱密码！'}
    if box_lock != '2': 
        return {'code':1,'msg':'购买前请先输入安全箱密码！'}

    '''
    flag add charge pwd

    charge_pwd = request.forms.get("charge_pwd","").strip()
    if not charge_pwd:
        return {'code':-4,'msg':'请先输入充值密码！'}
    real_cp = redis.hget('users:%s'%uid,'charge_pwd')
    if md5.new(charge_pwd).hexdigest()!=real_cp:
        return {'code':-3,'msg':'充值密码错误！'}
    '''
    ...
    '''
    goods_table = GOODS_TABLE%(goodId)
    user_table = 'users:%s'%uid
    cards,goodsName,present_card,goodsPrice = redis.hmget(goods_table,'cards','name','present_cards','price')

    redis.incrby('fish:diamond:charge:sum',goodsPrice)
    add_num = int(cards or 0) + int(present_card or 0)
    '''
    diamond = redis.hincrby('users:%s'%uid,'diamond',add_num)
    '''

    # 放进保管箱
    diamond = redis.hincrby('box:user:%s:hesh'%uid,'1',add_num)

    #redis.hincrby('users:%s'%uid,'warhead_bonus',add_num*10)

    #log_util.debug('[try buyCoin] return diamon[%s]'%(diamond))
    fish_pay_func.vip_charge_record(redis,uid,price)
    return {'code':0,'msg':'购买钻石成功', 'diamond':diamond}
