...
def get_mysql(host="127.0.0.1", user="root", password="168mysql",db="Match_db"):
    return MysqlPlugin(host=host,user=user,password=password,db=db)

def get_operate_db(host="120.79.135.194", user="root", password="168mysql"):
    return MysqlPlugin(host=host,user=user,password=password,db='operate_data')
